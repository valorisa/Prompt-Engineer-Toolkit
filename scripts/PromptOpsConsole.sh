...
show_prompt_optimizer() {
    echo ""
    echo -e "\e[36m🚀 Prompt Optimizer\e[0m"
    echo "----------------------------------------"
    echo "Optimize prompts for GPT, Claude, Gemini, Qwen"
    echo "Model-specific adaptations & best practices"
    echo ""
    read -rp "  Enter your prompt to optimize: " prompt
    if [[ -z "${prompt}" ]]; then
        echo -e "\e[33m  ⚠ No prompt entered\e[0m"
        log_action "Prompt Optimizer: Cancelled - no prompt"
        read -rp "  Press Enter to continue"
        return
    fi
    echo ""
    echo -e "\e[36m🎯 Target AI model:\e[0m"
    echo "  [1] GPT-4 / GPT-4o"
    echo "  [2] Claude 3 / 3.5"
    echo "  [3] Gemini Pro / Ultra"
    echo "  [4] Qwen 2.5 / Max"
    echo "  [0] Cancel"
    echo ""
    read -rp "  Select model: " model_choice
    local model_map=( ["1"]="gpt" ["2"]="claude" ["3"]="gemini" ["4"]="qwen" )
    if [[ "${model_choice}" == "0" ]]; then
        log_action "Prompt Optimizer: Cancelled by user"
        return
    fi
    if [[ -z "${model_map[${model_choice}]:-}" ]]; then
        echo -e "\e[33m  ⚠ Invalid model selection\e[0m"
        log_action "Prompt Optimizer: Invalid model selection"
        read -rp "  Press Enter to continue"
        return
    fi
    local selected_model="${model_map[${model_choice}]}"
    log_action "Prompt Optimizer: Optimizing for ${selected_model}"
    local node_path="${PROJECT_ROOT}/scripts/node"
    if [[ ! -d "${node_path}" ]]; then
        echo -e "\e[31m❌ Node.js CLI not found at: ${node_path}\e[0m"
        log_action "Prompt Optimizer: Node.js CLI not found"
        read -rp "  Press Enter to continue"
        return
    fi
    echo -e "\e[33m⏳ Optimizing your prompt...\e[0m"
    cd "${node_path}"
    npx tsx promptops.ts run prompt-optimizer --prompt="${prompt}" --model="${selected_model}" 2>&1
    local exit_code=$?
    cd "${PROJECT_ROOT}"
    if [[ ${exit_code} -eq 0 ]]; then
        echo -e "\e[32m✅ Optimization completed!\e[0m"
        log_action "Prompt Optimizer: Success for ${selected_model}"
    else
        echo -e "\e[33m⚠ Optimization completed with warnings\e[0m"
        log_action "Prompt Optimizer: Completed with warnings"
    fi
    read -rp "  Press Enter to continue"
}
...
while true; do
    ...
    case "${choice}" in
        ...
        "6")
            ...
            ;;
        "7")
            log_action "Menu: Prompt Optimizer selected"
            show_prompt_optimizer
            ;;
        "?")
        ...
        "0")
            ...
            ;;
        "")
            echo -e "\e[31m❌ Invalid option. Please choose 0-7 or ? for help.\e[0m"
